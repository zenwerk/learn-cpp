...
int main(int, char **) {
    ...
    for (;;) {
        int v;
        token = s.get(v);
        // 全体を受理したとき、もしくはエラーを検出したときにはtrueを、まだ受理が終わってない場合にはfalseを返す
        if (parser.post(token, v))
            break;
    }

    int v;
    // parser.acceptはparser.postがtrueを返して後でだけ有効。
    // errorの場合はvは不定になり、falseを返す。
    // 正常終了の場合はvにルート文法の左辺に関連付けられた値が代入され、trueを返す。
    if (parser.accept(v)) {
        std::cerr << "accpeted\n";
        std::cerr << v << std::endl;
    }
